...
const generateWAMessageContent = async (message, options) => {
    ...
    const interactiveMessage = {
        productCategoryMessage: {
            // struktur khusus untuk kategori produk
            surface: message.productCategory.product_category?.surface || 1,
            categoryId: message.productCategory.product_category?.category_id || 'cat_1437',
            businessOwnerJid: message.productCategory.product_category?.biz_jid || '0@s.whatsapp.net',

            // opsional tambahan meta dari konfigurasi (biar tetap fleksibel)
            title: message.productCategory.product_category?.title || '',
            description: message.productCategory.product_category?.description || '',
            buttonLabel: message.productCategory.product_category?.button_label || '',

            // bisa tetap bawa semua flow params JSON
