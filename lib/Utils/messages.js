...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
       ...
       m.eventMessage.contextInfo = {
        ...
        // 🔹 Thumbnail (support Buffer atau URL)
        if ('thumbnail' in message && message.thumbnail) {
            const mediaInput = Buffer.isBuffer(message.thumbnail) ?
                {
            ...
            const preparedHeader = await prepareWAMessageMedia(
                mediaInput, {
        ...
        // 🔹 Tambahkan dukungan productCategory
        if ('productCategory' in message) {
            interactiveMessage.nativeFlowMessage.productCategoryMessage = {
                surface: message.productCategory.surface || 1,
                categoryId: message.productCategory.id || 'default_category_id',
                businessOwnerJid: message.productCategory.bizJid || '0@s.whatsapp.net'
            }
        }
