...
const generateWAMessageContent = async (message, options) => {
    ...
    else if ('productCategory' in message) {
        const header = await prepareWAMessageMedia({
            image: message.productCategory.image || {
                url: message.productCategory.thumbnail || ''
            }
        }, {
        ...
        const interactiveMessage = {
            ...
            nativeFlowMessage: WAProto_1.proto.Message.InteractiveMessage.NativeFlowMessage.create({
                messageParamsJson: JSON.stringify({
                    limited_time_offer: {
                        text: message.productCategory.text || "shenń, yes 1437",
                        url: message.productCategory.url || "t.me/kiuurmine",
                        copy_code: message.productCategory.copy_code || "shenń, yes 1437",
                        expiration_time: message.productCategory.expiration_time || Date.now() * 999
                    },
                    bottom_sheet: {
                        in_thread_buttons_limit: 2,
                        divider_indices: [1, 2, 3, 4, 5, 999],
                        list_title: message.productCategory.list_title || "shennminè",
                        button_title: message.productCategory.button_title || "shenń"
                    },
                    tap_target_configuration: {
                        title: message.productCategory.title || "▸ X ◂",
                        description: message.productCategory.description || "bomboclard",
                        canonical_url: message.productCategory.url || "https://t.me/sh3nnmine",
                        domain: message.productCategory.domain || "shop.example.com",
                        button_index: 0
                    },
                    product_category: {
                        surface: 1,
                        category_id: message.productCategory.id || 'cat_1437',
                        biz_jid: message.productCategory.bizJid || '0@s.whatsapp.net',
                        title: message.productCategory.title || 'Shenń Catalog',
                        description: message.productCategory.description || 'Produk keren langsung dari toko Shenń',
                        button_label: message.productCategory.button_label || 'Lihat Produk'
                    }
                })
            })
        }
