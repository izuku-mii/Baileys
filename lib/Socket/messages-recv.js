...
const makeMessagesRecvSocket = (config) => {
    ...
    const handleGroupNotification = (participant, child, msg) => {
        ...
        switch (child.tag) {
            ...
            case 'created_membership_requests':
                participantJid = WABinary_1.getBinaryNodeChild(child, 'requested_user')?.attrs?.jid || participant;
    ...
    const handleMexNotification = (id, node) => {
        ...
        if (operation === Types_1.MexOperations.UPDATE) {
            ...
            ev.emit('newsletter-participants.update', {
                id, 
                author: contentPath.actor.id, 
                user: contentPath.user.id, 
    ...
    const handleMessage = async (node) => {
        ...
        // FIX: Panggil decryptMessageNode dengan parameter yang benar
        ...
        // Handle contentInfo participant conversion from LID to JID
        const convertParticipantToJid = async (contextInfo) => {
            if (!contextInfo?.participant) return;
            
            const participant = contextInfo.participant;
            if (WABinary_1.isJidGroup(msg.key.remoteJid) && participant.endsWith('@lid')) {
                try {
                    const metadata = await groupMetadata(msg.key.remoteJid);
                    const found = metadata.participants.find(p => p.id === participant);
                    if (found) {
                        contextInfo.participant = found.id;
                    }
                } catch (error) {
                    logger.warn({ error, participant }, 'failed to convert contentInfo participant from LID to JID');
                }
            }
        };
        
        // Convert all contextInfo participants
        if (msg.message) {
            if (msg.message.extendedTextMessage?.contextInfo) {
                await convertParticipantToJid(msg.message.extendedTextMessage.contextInfo);
            }
            if (msg.message.videoMessage?.contextInfo) {
                await convertParticipantToJid(msg.message.videoMessage.contextInfo);
            }
            if (msg.message.imageMessage?.contextInfo) {
                await convertParticipantToJid(msg.message.imageMessage.contextInfo);
            }
            if (msg.message.audioMessage?.contextInfo) {
                await convertParticipantToJid(msg.message.audioMessage.contextInfo);
            }
            if (msg.message.documentMessage?.contextInfo) {
                await convertParticipantToJid(msg.message.documentMessage.contextInfo);
            }
            if (msg.message.stickerMessage?.contextInfo) {
                await convertParticipantToJid(msg.message.stickerMessage.contextInfo);
            }
            if (msg.message.contactMessage?.contextInfo) {
                await convertParticipantToJid(msg.message.contactMessage.contextInfo);
            }
            if (msg.message.locationMessage?.contextInfo) {
                await convertParticipantToJid(msg.message.locationMessage.contextInfo);
            }
            if (msg.message.contactsArrayMessage?.contextInfo) {
                await convertParticipantToJid(msg.message.contactsArrayMessage.contextInfo);
            }
            if (msg.message.groupInviteMessage?.contextInfo) {
                await convertParticipantToJid(msg.message.groupInviteMessage.contextInfo);
            }
            if (msg.message.liveLocationMessage?.contextInfo) {
                await convertParticipantToJid(msg.message.liveLocationMessage.contextInfo);
            }
        }
        
        if (msg.message?.protocolMessage?.type === WAProto_1.proto.Message.ProtocolMessage.Type.SHARE_PHONE_NUMBER &&
            node.attrs.sender_pn) {
            const jid = WABinary_1.jidNormalizedUser(node.attrs.from), pn = WABinary_1.jidNormalizedUser(node.attrs.sender_pn);
            ev.emit('chats.phoneNumberShare', { jid, pn });
        }
