...
const makeMessagesRecvSocket = (config) => {
    ...
    const handleMessage = async (node) => {
        ...
        if (msg.message?.extendedTextMessage?.contextInfo?.participant) {
            ...
            if (WABinary_1.isJidGroup(msg.key.remoteJid) && participant.endsWith('@lid')) {
                try {
                    ...
                    if (found) {
                        msg.message.extendedTextMessage.contextInfo.participant = found.id;
                    }
                } catch (error) {
        ...
        // Handle other message types with contextInfo
        if (msg.message?.videoMessage?.contextInfo?.participant) {
            const participant = msg.message.videoMessage.contextInfo.participant;
            if (WABinary_1.isJidGroup(msg.key.remoteJid) && participant.endsWith('@lid')) {
                try {
                    const metadata = await groupMetadata(msg.key.remoteJid);
                    const found = metadata.participants.find(p => p.id === participant);
                    if (found) {
                        msg.message.videoMessage.contextInfo.participant = found.id;
                    }
                } catch (error) {
                    logger.warn({ error, participant }, 'failed to convert contentInfo participant from LID to JID');
                }
        ...
        if (msg.message?.imageMessage?.contextInfo?.participant) {
            ...
            if (WABinary_1.isJidGroup(msg.key.remoteJid) && participant.endsWith('@lid')) {
                try {
                    ...
                    const found = metadata.participants.find(p => p.id === participant);
                    if (found) {
                        msg.message.imageMessage.contextInfo.participant = found.id;
                    }
                } catch (error) {
                    logger.warn({ error, participant }, 'failed to convert contentInfo participant from LID to JID');
                }
        ...
        if (msg.message?.audioMessage?.contextInfo?.participant) {
            ...
            if (WABinary_1.isJidGroup(msg.key.remoteJid) && participant.endsWith('@lid')) {
                try {
                    ...
                    const found = metadata.participants.find(p => p.id === participant);
                    if (found) {
                        msg.message.audioMessage.contextInfo.participant = found.id;
                    }
                } catch (error) {
                    logger.warn({ error, participant }, 'failed to convert contentInfo participant from LID to JID');
                }
        ...
        if (msg.message?.documentMessage?.contextInfo?.participant) {
            ...
            if (WABinary_1.isJidGroup(msg.key.remoteJid) && participant.endsWith('@lid')) {
                try {
                    ...
                    const found = metadata.participants.find(p => p.id === participant);
                    if (found) {
                        msg.message.documentMessage.contextInfo.participant = found.id;
                    }
                } catch (error) {
                    logger.warn({ error, participant }, 'failed to convert contentInfo participant from LID to JID');
                }
        ...
        if (msg.message?.stickerMessage?.contextInfo?.participant) {
            const participant = msg.message.stickerMessage.contextInfo.participant;
            if (WABinary_1.isJidGroup(msg.key.remoteJid) && participant.endsWith('@lid')) {
                try {
                    const metadata = await groupMetadata(msg.key.remoteJid);
                    const found = metadata.participants.find(p => p.id === participant);
                    if (found) {
                        msg.message.stickerMessage.contextInfo.participant = found.id;
                    }
                } catch (error) {
                    logger.warn({ error, participant }, 'failed to convert contentInfo participant from LID to JID');
                }
        ...
        if (msg.message?.contactMessage?.contextInfo?.participant) {
            const participant = msg.message.contactMessage.contextInfo.participant;
            if (WABinary_1.isJidGroup(msg.key.remoteJid) && participant.endsWith('@lid')) {
                try {
                    const metadata = await groupMetadata(msg.key.remoteJid);
                    const found = metadata.participants.find(p => p.id === participant);
                    if (found) {
                        msg.message.contactMessage.contextInfo.participant = found.id;
                    }
                } catch (error) {
                    logger.warn({ error, participant }, 'failed to convert contentInfo participant from LID to JID');
                }
        ...
        if (msg.message?.locationMessage?.contextInfo?.participant) {
            const participant = msg.message.locationMessage.contextInfo.participant;
            if (WABinary_1.isJidGroup(msg.key.remoteJid) && participant.endsWith('@lid')) {
                try {
                    const metadata = await groupMetadata(msg.key.remoteJid);
                    const found = metadata.participants.find(p => p.id === participant);
                    if (found) {
                        msg.message.locationMessage.contextInfo.participant = found.id;
                    }
                } catch (error) {
                    logger.warn({ error, participant }, 'failed to convert contentInfo participant from LID to JID');
                }
