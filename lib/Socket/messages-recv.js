...
const makeMessagesRecvSocket = (config) => {
    ...
    const sendRetryRequest = async (node, forceIncludeKeys = false) => {
        const { fullMessage } = Utils_1.decodeMessageNode(node, authState.creds.me.id, '')
    ...
    const handleGroupNotification = (participant, child, msg) => {
        // Convert LID participant to regular JID
        let participantJid = WABinary_1.getBinaryNodeChild(child, 'participant')?.attrs?.jid || participant
        
        // If participant is LID, try to get phone number from attributes
        if (WABinary_1.isLidUser(participantJid)) {
            participantJid = WABinary_1.getBinaryNodeChild(child, 'participant')?.attrs?.phone_number || participant
        }
        ...
        switch (child.tag) {
            ...
            case 'not_ephemeral':
                ...
                break
            case 'modify':
                const oldNumber = WABinary_1.getBinaryNodeChildren(child, 'participant').map(p => 
                    p.attrs.phone_number || p.attrs.jid // Prioritize phone number over JID
                )
            ...
            case 'leave':
                ...
                const participants = WABinary_1.getBinaryNodeChildren(child, 'participant').map(p => 
                    p.attrs.phone_number || p.attrs.jid // Prioritize phone number over JID
                )
            ...
            case 'created_membership_requests':
                participantJid = WABinary_1.getBinaryNodeChild(child, 'requested_user')?.attrs?.phone_number || 
                               WABinary_1.getBinaryNodeChild(child, 'requested_user')?.attrs?.jid || 
                               participant
    ...
    const handleMexNotification = (id, node) => {
        ...
        if (operation === Types_1.MexOperations.UPDATE) {
            ...
            ev.emit('limit-sharing.update', {
            	id, 
                author: contentPath.updated_by?.jid, 
            ...
            ev.emit('community-owner.update', {
            	id, 
                author: contentPath.updated_by?.jid, 
                user: contentPath.role_updates[0].user?.jid, 
            ...
            ev.emit('newsletter-participants.update', {
            	id, 
                author: contentPath.actor.jid, 
                user: contentPath.user.jid, 
    ...
    const processNotification = async (node) => {
        ...
        switch (nodeType) {
            ...
            case 'w:gp2':
                ...
                if (WABinary_1.isLidUser(participant)) {
                    participant = node.attrs.participant_pn || participant
                }
            ...
            case 'devices':
                ...
                if (WABinary_1.areJidsSameUser(child.attrs.jid, authState.creds.me.id)) {
                    const deviceData = devices.map(d => ({ id: d.attrs.jid }))
    ...
    const handleReceipt = async (node) => {
        ...
        // Always use regular JID, not LID
        const isNodeFromMe = WABinary_1.areJidsSameUser(attrs.participant || attrs.from, authState.creds.me?.id)
...
const handleMessage = async (node) => {
    ...
    if (WABinary_1.getBinaryNodeChild(node, 'unavailable') && !encNode) {
        await sendMessageAck(node)
        const { key } = Utils_1.decodeMessageNode(node, authState.creds.me.id, '').fullMessage
    ...
    const { fullMessage: msg, category, author, decrypt } = Utils_1.decryptMessageNode(node, authState.creds.me.id, '', signalRepository, logger)

    if (response && msg?.messageStubParameters?.[0] === Utils_1.NO_MESSAGE_FOUND_ERROR_TEXT) {
        msg.messageStubParameters = [Utils_1.NO_MESSAGE_FOUND_ERROR_TEXT, response]
    }

    // Convert LID to regular JID for message sender
    if (WABinary_1.isLidUser(msg.key.remoteJid)) {
        msg.key.remoteJid = node.attrs.sender_pn || node.attrs.peer_recipient_pn || msg.key.remoteJid
    }

    // Convert LID participant to regular JID
    if (msg.key.participant && WABinary_1.isLidUser(msg.key.participant)) {
        msg.key.participant = node.attrs.participant_pn || msg.key.participant
    }
    ...
    try {
        await Promise.all([
            processingMutex.mutex(async () => {
                ...
                // **TAMBAHKAN KODE INI UNTUK KONVERSI contextInfo.participant DARI LID KE JID**
                // Handle contextInfo participant conversion for quoted messages
                const convertContextInfoParticipant = (contextInfo) => {
                    if (contextInfo?.participant && WABinary_1.isLidUser(contextInfo.participant)) {
                        // Use sender_pn or participant_pn to get the regular JID
                        const jid = node.attrs.sender_pn || node.attrs.participant_pn || contextInfo.participant
                        if (!WABinary_1.isLidUser(jid)) {
                            contextInfo.participant = jid
                            logger.debug({ 
                                original: contextInfo.participant, 
                                converted: jid 
                            }, 'Converted contextInfo.participant from LID to JID')
                        }
                    }
                }

                // Convert contextInfo.participant for extendedTextMessage (quoted messages)
                if (msg.message?.extendedTextMessage?.contextInfo) {
                    convertContextInfoParticipant(msg.message.extendedTextMessage.contextInfo)
                }

                // Convert contextInfo.participant for other message types
                if (msg.message?.imageMessage?.contextInfo) {
                    convertContextInfoParticipant(msg.message.imageMessage.contextInfo)
                }
                if (msg.message?.videoMessage?.contextInfo) {
                    convertContextInfoParticipant(msg.message.videoMessage.contextInfo)
                }
                if (msg.message?.audioMessage?.contextInfo) {
                    convertContextInfoParticipant(msg.message.audioMessage.contextInfo)
                }
                if (msg.message?.documentMessage?.contextInfo) {
                    convertContextInfoParticipant(msg.message.documentMessage.contextInfo)
                }
                if (msg.message?.stickerMessage?.contextInfo) {
                    convertContextInfoParticipant(msg.message.stickerMessage.contextInfo)
                }
